...
export function createLogicGuardService() {
  ...
  function mutateProb(state: GameState, logic: CouncilLogic): boolean {
  ...
  function mutateLuckyRatio(state: GameState, logic: CouncilLogic): boolean {
    if (logic.targetType === "proposed") {
      return game.isEffectMutable(state, logic.targetCondition - 1);
    }
    return true;
  ...
  // 대신 기회를 2회 소모하겠군.
  function decreaseTurnLeft(state: GameState, logic: CouncilLogic): boolean {
    return state.turnLeft > 1;
  }

  // <모든 효과>의 단계를 뒤섞도록 하지. 어떻게 뒤섞일지 보자고.
  function shuffleAll(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }
  ...
  function unsealAndSealOther(state: GameState, logic: CouncilLogic): boolean {
    return !!state.effects.find((eff) => eff.isSealed);
  }

  // <네가 고르는> 슬롯의 효과를 바꿔주지. 어떤 효과일지 보자고.
  function changeEffect(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }

  // <{0}> 효과를 봉인하겠다.
  function sealTarget(state: GameState, logic: CouncilLogic): boolean {
  ...
  function increaseReroll(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }

  // 남은 모든 연성에서 비용이 <20%> 덜 들겠어.
  function decreasePrice(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }

  // 이대론 안되겠어. 엘릭서의 효과와 단계를 <초기화>하겠다.
  function restart(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }
  ...
  function setValueRanged(state: GameState, logic: CouncilLogic): boolean {
    if (logic.targetType === "proposed") {
      return game.isEffectMutable(state, logic.targetCondition - 1);
    }
    return true;
  ...
  function redistributeAll(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }
  ...
  function shiftAll(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }

  // <{0}> 효과와 <{1}> 효과의 단계를 뒤바꿔줄게.
  function swapValues(state: GameState, logic: CouncilLogic): boolean {
    return (
  ...
  function swapMinMax(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }

  // 소진
  function exhaust(state: GameState, logic: CouncilLogic): boolean {
    return true;
  }
  ...
  const logicGuards: Record<
    CouncilLogicType,
    (state: GameState, logic: CouncilLogic) => boolean
  > = {
    mutateProb,
    mutateLuckyRatio,
  ...
  function runLogicGuard(state: GameState, logic: CouncilLogic): boolean {
